...
function post_family_config__napi2_overlay_prefix_and_defaults() {
	declare -g OVERLAY_PREFIX="rk3568"
	declare -g TZDATA="Europe/Moscow"
	declare -g INSTALL_HEADERS="${INSTALL_HEADERS:-yes}"
}

function post_family_tweaks_bsp__napi_bsp_cli_files_2() {
...
function post_family_tweaks__napi2_provision_accounts() {
...
function post_family_tweaks__napi_motd_no_wan_lookup_2() {
...
function post_family_tweaks__napi2_desktop_assets() {
	...
	if [[ -d "${SRC}/packages/bsp/napi-desktop" ]]; then
		...
		# the bsp-cli copy below, nothing chowns ${SDCARD} afterwards. tar keeps the
...
function post_repo_customize_image__napi_extra_repo_packages_2() {
	if [[ -f "${SDCARD}/usr/share/keyrings/napilab.gpg" ]]; then
		...
		# entry in packages/bsp/napi/etc/apt/preferences.d/napilab lets apt replace (and
		# even downgrade) the distro packages with the napilab ones on its own.
...
function image_specific_armbian_env_ready__napi2_extraargs() {
...
function post_family_tweaks__napi2_prune_dtb() {
	declare d
	for d in "${SDCARD}"/boot/dtb-*; do
		...
		find "${d}/rockchip" -maxdepth 1 -type f -name '*.dtb' ! -name 'rk3568-napi2.dtb' -delete
